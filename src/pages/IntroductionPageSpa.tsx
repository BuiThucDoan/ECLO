...
import CardDetailSpa from '../components/card/CardDetailSpa';


type MenuItem = Required<MenuProps>['items'][number];//định nghĩa kiểu dữ liệu
const items: MenuItem[] = findAnswerSpa.map((topic) => ({
...
export default function IntroductionPageSpa() {
  ...
  const handleMenuClick: MenuProps['onClick'] = (e) => {
    ...
    if (item?.link) {
      navigate(`/chitiet-SPA/${item?.link}`);
    }
  };
    ...
    if (item) {
      ...
      if (parentItem) {
        newBreadcrumb.push({
          ...
          link: `/chitiet-SPA/${parentItem.link}`,
        });
      }
  ...
  const findItem = findAnswerSpa.find((item: ITopic) => item.link === params.name);
