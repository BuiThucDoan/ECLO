...
import { instructionDetailERP } from '../mock/detailerp';
...
export default function IntroductionDetail() {
  ...
  const findDetail = instructionDetailERP.find(
  ...
  useEffect(() => {
    ...
    if (findDetail) {
      ...
      const buildBreadcrumb = (link) => {
        const parent = findParentItem(instructionDetailERP, link);
        if (parent) {
          buildBreadcrumb(parent.link);
          newBreadcrumb.push({
            title: parent.label,
            link: `/chitiet-ERP/${parent.link}`,
          });
        }
      };
      ...
      newBreadcrumb.push({
        ...
        link: `/chitiet-ERP/${findDetail.link}`,
      });
      ...
      <div className="w-4/5 flex h-full justify-between flex-row mt-2 bg-gray-0">
        ...
        <div className="w-1/5 flex h-full flex-col overflow-auto ml-100 font-be-vietnam-pro hide-on-mobile fixed right-0 mr-12">
          ...
          <div className="border-l-2 border-gray-200 related-articles">
            ...
            <a
              href="huong-dan-su-dung-trang-cong-viec"
              className="font-plus-jakarta ml-4 mt-3 text-sm block sm:inline-block">
              <FontAwesomeIcon icon={faFileAlt} className="mr-1" /> Công việc
            </a>
            <br />
            <a
              href="huong-dan-su-dung-trang-de-xuat"
              className="font-plus-jakarta ml-4 mt-3 text-sm block sm:inline-block">
              <FontAwesomeIcon icon={faFileAlt} className="mr-1 mt-1" /> Đề xuất
            </a>
            <br />
            <a
              href="huong-dan-su-dung-trang-cham-cong"
              className="font-plus-jakarta ml-4 mt-3 text-sm block sm:inline-block">
              <FontAwesomeIcon icon={faFileAlt} className="mr-1 pt-1" /> Chấm công
            </a>
            <br/>
            <a
              href="huong-dan-su-dung-trang-nhan-vien"
              className="font-plus-jakarta ml-4 mt-3 text-sm block sm:inline-block">
              <FontAwesomeIcon icon={faFileAlt} className="mr-1 pt-1" /> Quản lý nhân viên
            </a>
          </div>
          <br />
          <div className="border-b-2 border-gray-200"></div>
