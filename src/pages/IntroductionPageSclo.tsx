...
import CardDetailSclo from '../components/card/CardDetailSclo';


type MenuItem = Required<MenuProps>['items'][number];//định nghĩa kiểu dữ liệu
const items: MenuItem[] = findAnswerSclo.map((topic) => ({
...
export default function IntroductionPageSclo() {
  ...
  const handleMenuClick: MenuProps['onClick'] = (e) => {
    ...
    if (item?.link) {
      navigate(`/chitiet-SCLO/${item?.link}`);
    }
  };
    ...
    if (item) {
      ...
      if (parentItem) {
        newBreadcrumb.push({
          ...
          link: `/chitiet-SCLO/${parentItem.link}`,
        });
      }
      newBreadcrumb.push({ title: item.label, link: `/detail2/${item.link}` });
      setBreadCrumb(newBreadcrumb);
    }
  };

  const findItem = findAnswerSclo.find((item: ITopic) => item.link === params.name);
