...
import { instructionDetail } from '../mock/detail';
...
export default function IntroductionDetail() {
  ...
  const findDetail = instructionDetail.find(
  ...
  useEffect(() => {
    ...
    if (findDetail) {
      ...
      const buildBreadcrumb = (link) => {
        const parent = findParentItem(instructionDetail, link);
        if (parent) {
          buildBreadcrumb(parent.link);
          newBreadcrumb.push({
            title: parent.label,
            link: `/chitiet-GOLD/${parent.link}`,
          });
        }
      };
      ...
      newBreadcrumb.push({
        ...
        link: `/chitiet-GOLD/${findDetail.link}`,
      });
      ...
      <div className="w-4/5 flex h-full justify-between flex-row mt-2 bg-gray-0">
        ...
        <div className="w-1/5 flex h-full flex-col overflow-auto ml-100 font-be-vietnam-pro hide-on-mobile fixed right-0 mr-12">
          ...
          <div className="border-l-2 border-gray-200 related-articles">
            ...
            <a
              href="huong-dan-su-dung-trang-tem"
              className="font-plus-jakarta ml-4 mt-3 text-sm block sm:inline-block">
              <FontAwesomeIcon icon={faFileAlt} className="mr-1" /> Hướng dẫn sử dụng Tem
            </a>{' '}
            <br />
            <a
              href="huong-dan-cau-hinh-can"
              className="font-plus-jakarta ml-4 mt-3 text-sm block sm:inline-block">
              <FontAwesomeIcon icon={faFileAlt} className="mr-1 mt-1" /> Hướng dẫn cấu hình Cân
            </a>{' '}
            <br />
            <a
              href="huong-dan-ket-noi-can"
              className="font-plus-jakarta ml-4 mt-3 text-sm block sm:inline-block">
              <FontAwesomeIcon icon={faFileAlt} className="mr-1 mt-1" /> Hướng dẫn kết nối Cân
            </a>{' '}
            <br />
            <a
              href="huong-dan-su-dung-trang-cau-hinh"
              className="font-plus-jakarta ml-4 mt-3 text-sm block sm:inline-block">
              <FontAwesomeIcon icon={faFileAlt} className="mr-1 pt-1" /> Hướng dẫn sử dụng Cấu hình
            </a>
          </div>
          <br />
          <div className="border-b-2 border-gray-200"></div>
