...
import CardDetailERP from '../components/card/CardDetailERP';


type MenuItem = Required<MenuProps>['items'][number];//định nghĩa kiểu dữ liệu
const items: MenuItem[] = findAnswerERP.map((topic) => ({
...
export default function IntroductionPageERP() {
  ...
  const handleMenuClick: MenuProps['onClick'] = (e) => {
    ...
    if (item?.link) {
      navigate(`/chitiet-HRM/${item?.link}`);
    }
  };
    ...
    if (item) {
      ...
      if (parentItem) {
        newBreadcrumb.push({
          ...
          link: `/chitiet-HRM/${parentItem.link}`,
        });
      }
      newBreadcrumb.push({ title: item.label, link: `/chitiet-ERP/${item.link}` });
      setBreadCrumb(newBreadcrumb);
    }
  };

  const findItem = findAnswerERP.find((item: ITopic) => item.link === params.name);
