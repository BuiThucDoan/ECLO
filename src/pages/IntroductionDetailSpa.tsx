...
import { instructionDetailSpa } from '../mock/detailspa';
...
export default function IntroductionDetail() {
  ...
  const findDetail = instructionDetailSpa.find(
  ...
  useEffect(() => {
    ...
    if (findDetail) {
      ...
      const buildBreadcrumb = (link) => {
        const parent = findParentItem(instructionDetailSpa, link);
        if (parent) {
          buildBreadcrumb(parent.link);
          newBreadcrumb.push({
            title: parent.label,
            link: `/chitiet-SPA/${parent.link}`,
          });
        }
      };
      ...
      newBreadcrumb.push({
        ...
        link: `/chitiet-SPA/${findDetail.link}`,
      });
  ...
  return (
    <div className="w-full h-full flex items-center flex-col justify-center">
      ...
      <div className="w-4/5 flex h-full justify-between flex-row mt-2 bg-gray-0">
        ...
        <div className="w-1/5 flex h-full flex-col overflow-auto ml-100 font-be-vietnam-pro hide-on-mobile fixed right-0 mr-12">
          <Anchor
            className="bg-white"
          ...
          <div className="border-l-2 border-gray-200 related-articles">
            ...
            <a
              href="huong-dan-su-dung-kho-thanh-pham-spa"
              className="font-plus-jakarta ml-4 mt-3 text-sm block sm:inline-block">
              <FontAwesomeIcon icon={faFileAlt} className="mr-1" /> Kho thành phẩm
            </a>
            <br />
            <a
              href="huong-dan-su-dung-kho-nguyen-lieu-spa"
              className="font-plus-jakarta ml-4 mt-3 text-sm block sm:inline-block">
              <FontAwesomeIcon icon={faFileAlt} className="mr-1 mt-1" /> Kho nguyên liệu
            </a>
            <br />
            <a
              href="huong-dan-su-dung-trang-lieu-trinh"
              className="font-plus-jakarta ml-4 mt-3 text-sm block sm:inline-block">
              <FontAwesomeIcon icon={faFileAlt} className="mr-1 pt-1" /> Liệu trình
            </a>
            <br/>
            <a
              href="huong-dan-su-dung-trang-dat-truoc"
              className="font-plus-jakarta ml-4 mt-3 text-sm block sm:inline-block">
              <FontAwesomeIcon icon={faFileAlt} className="mr-1 pt-1" /> Đặt trước
            </a>
          </div>
          <br />
          <div className="border-b-2 border-gray-200"></div>
